...
export interface IPostService {
  ...
  ) => Promise<Result<PostResponse, ApiError>>;
  getPostsForUser: (
    request: GetPostsForUserRequest
  ) => Promise<Result<PostResponse[], ApiError>>;
}
...
export const postService: IPostService = {
  ...
  uploadPostImage: async function (
    request: FormData
  ...
  getPostsForUser: async function (
    request: GetPostsForUserRequest
  ): Promise<Result<PostResponse[], ApiError>> {
    return await executeApiCall(async function () {
      const response = await axiosInstance.get<PostResponse[]>(
        `posts/user/${request.username}`
      );
      return response.data;
    });
  },
