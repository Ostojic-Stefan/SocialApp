...
import {
  ...
  AddCommentResponse,
  CommentResponse,
  CommentsFromPostResponse,
  commentService,
} from '../../api/commentService';
import { ApiError } from '../../api/models';
import { increaseCommentCount } from '../posts/postSlice';
...
export const addCommentToAPost = createAsyncThunk<AddCommentResponse, AddCommentRequest, { rejectValue: ApiError }>(
  ...
  async function (data, { dispatch, rejectWithValue }) {
    ...
    dispatch(increaseCommentCount({ postId: data.postId }));
    return response.value;
  }
);
...
const commentsSlice = createSlice({
  ...
  reducers: {
    addComment(state, action) {
      state.data.find((d) => d.postId === action.payload.postId)?.comments.push(action.payload.comment);
    },
  },
